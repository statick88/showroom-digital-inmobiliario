  ...
  return (
    <section className="relative w-full h-[400px] sm:h-[500px] rounded-2xl overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10">
        {nombre && (
          <h1 className="typo-headline-lg text-white mb-2 drop-shadow-lg">
            {nombre}
          </h1>
        )}
        {descripcion && (
          <p className="typo-body-lg text-white/90 max-w-xl drop-shadow">
            {descripcion}
          </p>
        )}
        {imagenes360 && imagenes360.length > 0 && (
          <button className="mt-4 inline-flex items-center gap-2 bg-white/20 backdrop-blur-sm text-white px-5 py-2.5 rounded-xl typo-label-md font-bold hover:bg-white/30 transition-all">
